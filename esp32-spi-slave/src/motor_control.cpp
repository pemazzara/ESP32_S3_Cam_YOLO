...
void MotorControl::setPWM(int16_t pwm, int16_t targetAngle, bool immediate) {
    ...
    // Normalizar targetAngle a [0, 360)
    int16_t normalizedTarget = targetAngle % 360;
...
if (currentInterpolatedAngle >= 360.0f) currentInterpolatedAngle -= 360.0f;
    ...
    // El ángulo 0° debe ser DERECHA (no adelante)
    // El ángulo 90° debe ser ADELANTE
    // El ángulo 180° debe ser IZQUIERDA
    // El ángulo 270° debe ser ATRÁS
    
    // Convertir ángulo para que:
    // 90°=adelante (forward=1, turn=0)
    // 270°=atrás (forward=-1, turn=0)
    // 0°=derecha (forward=0, turn=1)
    // 180°=izquierda (forward=0, turn=-1)
    
    float adjustedRad = (currentInterpolatedAngle - 90.0f) * M_PI / 180.0;
    
    float forwardComponent = cos(adjustedRad);  // +1=adelante, -1=atrás
    float turnComponent = sin(adjustedRad);     // +1=izquierda, -1=derecha
    
    // Cálculo de velocidades diferenciales
    int16_t rawLeft = (int16_t)((forwardComponent + turnComponent) * pwm);
    int16_t rawRight = (int16_t)((forwardComponent - turnComponent) * pwm);
    ...
    Serial.printf("🔧Aplicando LeftSpeed=%d, RightSpeed=%d\n", 
              targetLeftSpeed, targetRightSpeed);
    applyHardwarePWM(currentLeftSpeed, currentRightSpeed);
